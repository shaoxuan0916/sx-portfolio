  ...
  return (
    <div>
      {timeline.map((item, index) => (
        <div
          className="flex md:grid md:grid-cols-4 flex-col space-y-10 md:space-y-0 space-x-4 my-20 relative"
          key={`timeline-${index}`}
        >
          <Paragraph className="w-40 md:col-span-1">{item.date}</Paragraph>
          <div className="md:col-span-3">
            <Heading
              as="h5"
              className="text-lg md:text-lg lg:text-lg text-emerald-500 mb-2"
            >
              {item.company}
            </Heading>
